/**
 * Vendored and adapted from Brhiza/mingyu packages/core under the MIT License.
 * Copyright (c) 2025 mingyu.
 * Upstream commit: bd6963b9b562cbef77c50227b625c0d3e7b36021
 * Upstream path: packages/core/src/ziwei/iztro/pattern-detection.ts
 * Adaptation: traditional outcome interpretations and source quotations are removed; detection conditions and source locators remain.
 */
...
export const ZIWEI_TRADITIONAL_PATTERN_BOUNDARIES = deepFreeze([
    {
        name: '禄马佩印',
...
const TEMPLE_OR_PROSPEROUS_BRIGHTNESS = new Set([
    '庙',
    '旺'
]);
const FOUR_MALEFICS = [
...
function getSurroundedPalaces(context, palace) {
    return palace.surrounded_palace_indexes.map((index)=>context.palaceByIndex.get(index)).filter((item)=>!!item);
}
...
function getFlankingMatch(context, palace, firstStar, secondStar) {
    ...
    const matched = hasStar(neighbors[0], firstStar) && hasStar(neighbors[1], secondStar) || hasStar(neighbors[0], secondStar) && hasStar(neighbors[1], firstStar);
...
export const VERIFIED_PATTERN_RULES = deepFreeze([
    {
    ...
    },
    {
        ...
        detect (context) {
            ...
            if (neighbors.length === 2 && (hasStar(neighbors[0], '左辅') && hasStar(neighbors[1], '右弼') || hasStar(neighbors[0], '右弼') && hasStar(neighbors[1], '左辅'))) {
                return {
                    palaces: [
                        soulPalace,
                        ...neighbors
    ...
    },
    {
        ...
        detect (context) {
            ...
            if (neighbors.length !== 2) return null;
            const matched = hasStar(neighbors[0], '左辅') && hasStar(neighbors[1], '右弼') || hasStar(neighbors[0], '右弼') && hasStar(neighbors[1], '左辅');
            return matched ? {
                palaces: [
                    context.soulPalace,
                    ...neighbors
    ...
    },
    {
        ...
        detect (context) {
            ...
            const matched = hasStar(context.soulPalace, '天魁') && hasStar(opposite, '天钺') || hasStar(context.soulPalace, '天钺') && hasStar(opposite, '天魁');
            return matched ? {
                palaces: [
                    context.soulPalace,
                    opposite
    ...
    },
    {
        ...
        detect (context) {
            const target = context.palaces.find((palace)=>(palace.index === context.soulPalace.index || palace.is_body_palace) && hasAllStars(palace, [
                    '文曲',
                    '武曲'
                ]));
            return target ? {
    ...
    },
    {
        ...
        detect ({ soulPalace }) {
            ...
            const voidStars = [
                '地空',
                '地劫'
            ].filter((name)=>hasStar(soulPalace, name));
    ...
    },
    {
        ...
        detect (context) {
            const targets = [
                context.soulPalace,
                getPalaceByName(context, '官禄')
            ].filter((palace)=>!!palace && palace.earthly_branch === '卯' && hasStar(palace, '太阳'));
    ...
    },
    {
        ...
        detect (context) {
            ...
            return target && hasAllStars(target, [
                '太阳',
                '文昌'
            ]) ? {
                palaces: [
    ...
    },
    {
        ...
        detect (context) {
            const targets = context.palaces.filter((palace)=>(palace.index === context.soulPalace.index || palace.is_body_palace) && hasAllStars(palace, [
                    '禄存',
                    '天马'
                ]));
            return targets.length ? {
    ...
    },
    {
        ...
        detect (context) {
            ...
            const neighbors = getNeighborPalaces(context, context.soulPalace);
            if (neighbors.length !== 2) return null;
            const matched = hasStar(neighbors[0], '武曲') && hasStar(neighbors[1], '禄存') || hasStar(neighbors[0], '禄存') && hasStar(neighbors[1], '武曲');
            return matched ? {
                palaces: [
                    context.soulPalace,
                    ...neighbors
    ...
    },
    {
        ...
        detect (context) {
            ...
            return target && hasAllStars(target, [
                ...
                conditions: [
                    '太阳与太阴同守田宅宫',
                    `田宅宫在${target.earthly_branch}${[
                        '辰',
                        '戌',
                        '丑',
                        '未'
                    ].includes(target.earthly_branch) ? '，属于古籍所喜墓库' : '，不附加墓库条件'}`
                ]
            } : null;
        }
    },
    {
        ...
        detect ({ soulPalace }) {
            return [
                '子',
                '午'
            ].includes(soulPalace.earthly_branch) && hasStar(soulPalace, '巨门') ? {
                palaces: [
                    soulPalace
                ],
                stars: [
                ...
                conditions: [
                    `巨门在${soulPalace.earthly_branch}宫守命`
                ]
            } : null;
        }
    },
    ...
    },
    {
        ...
        detect (context) {
            if (![
                '寅',
                '申'
            ].includes(context.soulPalace.earthly_branch)) return null;
            const required = [
                '天机',
                '太阴',
                '天同',
    ...
    },
    {
        ...
        detect (context) {
            ...
            const matched = hasStar(wealth, '天府') && hasStar(career, '天相') || hasStar(wealth, '天相') && hasStar(career, '天府');
            return matched ? {
                palaces: [
                    context.soulPalace,
                    wealth,
    ...
    },
    {
        ...
        detect (context) {
            ...
            const literaryStars = target ? getMatchedStarNames(target, [
                '文昌',
                '文曲'
            ]) : [];
    ...
    },
    {
        ...
        detect (context) {
            ...
            return sunPalace && moonPalace && isTempleOrProsperous(getStar(sunPalace, '太阳')) && isTempleOrProsperous(getStar(moonPalace, '太阴')) ? {
                palaces: uniquePalaces([
                    context.soulPalace,
                    sunPalace,
                    moonPalace
    ...
    },
    {
        ...
        detect (context) {
            ...
            return target && [
                '子',
                '午'
            ].includes(target.earthly_branch) && hasStar(target, '禄存') ? {
                palaces: [
                    context.soulPalace,
                    target
                ],
                ...
                conditions: [
                    `禄存在${target.earthly_branch}宫守迁移，对照命宫`
                ]
            } : null;
        }
    },
    ...
    },
    {
        ...
        detect ({ soulPalace }) {
            return [
                ...
                '子',
                '午'
            ].includes(soulPalace.earthly_branch) && hasStar(soulPalace, '七杀') ? {
                palaces: [
                    soulPalace
                ],
                stars: [
                ...
                conditions: [
                    `七杀在${soulPalace.earthly_branch}宫守命`
                ]
            } : null;
        }
    },
    {
        ...
        detect ({ soulPalace }) {
            return isTempleOrProsperous(getStar(soulPalace, '贪狼')) && isTempleOrProsperous(getStar(soulPalace, '火星')) ? {
                palaces: [
                    soulPalace
                ],
                stars: [
    ...
    },
    {
        ...
        detect ({ soulPalace }) {
            const huaQuan = getBirthMutagenStars(soulPalace, [
                '权'
            ]).find(isTempleOrProsperous);
            const huaLu = getBirthMutagenStars(soulPalace, [
                '禄'
            ]).find(isTempleOrProsperous);
    ...
    },
    {
        ...
        detect (context) {
            const target = getSoulAndBodyPalaces(context).find((palace)=>hasAllStars(palace, [
                    '天刑',
                    '廉贞'
                ]));
            return target ? {
    ...
    },
    {
        ...
        detect ({ soulPalace }) {
            return [
                '申',
                '未'
            ].includes(soulPalace.earthly_branch) && hasStar(soulPalace, '廉贞') && !hasAnyMalefic([
                soulPalace
            ]) ? {
                palaces: [
                    soulPalace
                ...
                conditions: [
                    `廉贞在${soulPalace.earthly_branch}宫守命`,
                    '命宫不见羊陀火铃'
                ]
            } : null;
        }
    },
    {
        ...
        detect ({ soulPalace }) {
            return [
                '子',
                '午'
            ].includes(soulPalace.earthly_branch) && hasStar(soulPalace, '破军') && !hasAnyMalefic([
                soulPalace
            ]) ? {
                palaces: [
                    soulPalace
                ...
                conditions: [
                    `破军在${soulPalace.earthly_branch}宫守命`,
                    '命宫不见羊陀火铃'
                ]
            } : null;
        }
    ...
    },
    {
        ...
        detect (context) {
            const target = getSurroundedPalaces(context, context.soulPalace).find((palace)=>hasStar(palace, '禄存') && hasVoidStar(palace) && getMatchedStarNames(palace, KONG_JIE_STARS).length > 0);
            ...
            return {
                ...
                conditions: [
                    `禄存与${voidStars.join('、')}及${kongJie.join('、')}同宫`
                ]
            };
        }
    },
    ...
    },
    {
        ...
        detect (context) {
            ...
            const matched = hasStar(context.soulPalace, '武曲') && hasStar(body, '廉贞') || hasStar(context.soulPalace, '廉贞') && hasStar(body, '武曲');
            return matched ? {
                palaces: [
                    context.soulPalace,
                    body
    ...
    },
    {
        ...
        detect (context) {
            const target = getSoulAndBodyPalaces(context).find((palace)=>natalStars(palace).some((star)=>FOUR_MALEFICS.includes(star.name) && star.brightness === '陷'));
            if (!target) return null;
            const fallen = natalStars(target).filter((star)=>FOUR_MALEFICS.includes(star.name) && star.brightness === '陷').map((star)=>star.name);
            return {
                palaces: [
                    target
                ],
                stars: fallen,
                conditions: [
                    `${fallen.join('、')}以“陷”亮度守${target.name}`
                ]
            };
        }
    },
    {
        ...
        detect (context) {
            const huaJi = getBirthMutagenStars(context.soulPalace, [
                '忌'
            ]);
    ...
    },
    {
        ...
        detect ({ soulPalace }) {
            const malefics = getMatchedStarNames(soulPalace, [
                '擎羊',
                '陀罗'
            ]);
            return [
                '亥',
                '子'
            ].includes(soulPalace.earthly_branch) && hasStar(soulPalace, '贪狼') && malefics.length > 0 ? {
                palaces: [
                    soulPalace
                ],
                stars: [
    ...
    },
    {
        ...
        detect ({ soulPalace }) {
            return isTempleOrProsperous(getStar(soulPalace, '天梁')) && isTempleOrProsperous(getStar(soulPalace, '文昌')) ? {
                palaces: [
                    soulPalace
                ],
                stars: [
    ...
    },
    {
        ...
        detect (context) {
            for (const target of getSoulAndBodyPalaces(context)){
                ...
                const forbidden = surrounded.flatMap((palace)=>getMatchedStarNames(palace, [
                        '擎羊',
                        '陀罗',
                        '铃星'
                    ]));
    ...
    },
    {
        ...
        detect ({ soulPalace }) {
            return soulPalace.earthly_branch === '寅' && hasAllStars(soulPalace, [
                '贪狼',
                '陀罗'
            ]) ? {
                palaces: [
    ...
    },
    {
        ...
        detect (context) {
            return context.soulPalace.earthly_branch === '卯' && hasAllStars(context.soulPalace, [
                '巨门',
                '天机'
                ...
                '己',
                '丙'
            ].includes(context.birthYearHeavenlyStem) ? {
                palaces: [
                    context.soulPalace
                ],
                stars: [
                ...
                conditions: [
                    '巨门、天机同守卯宫命宫',
                    `生年天干为${context.birthYearHeavenlyStem}`
                ]
            } : null;
        }
    }
...
function assertValidPatternPalaces(palaces) {
    ...
    palaces.forEach((palace, position)=>{
        if (!Number.isInteger(palace?.index) || palace.index < 0 || palace.index >= ZIWEI_PALACE_COUNT) {
    ...
    palaces.forEach((palace)=>{
        if (!Number.isInteger(palace.opposite_palace_index) || !seenIndexes.has(palace.opposite_palace_index)) {
            throw new Error(`紫微格局检测${palace.name}对宫索引无效。`);
        }
        if (!Array.isArray(palace.surrounded_palace_indexes) || palace.surrounded_palace_indexes.length === 0) {
...
function createPatternContext(params) {
    ...
    return {
        ...
        palaceByIndex: new Map(params.palaces.map((palace)=>[
                palace.index,
                palace
            ])),
        soulPalace,
...
export const ZIWEI_PATTERN_AUDIT_NOTICE = `原有${RETIRED_UNVERIFIED_PATTERN_COUNT}条项目格局规则已全部退役；固定版本传统目录现登记${ZIWEI_TRADITIONAL_PATTERN_CATALOG_COUNT}项，其中${VERIFIED_ZIWEI_PATTERN_RULE_COUNT}条具备卷次定位和可复算条件，${ZIWEI_TRADITIONAL_PATTERN_BOUNDARIES.length}项明确登记为不可唯一复算边界`;
...
export function selectVerifiedZiweiPatterns(params) {
    ...
    const requestedStableKeys = new Set(params.patterns.flatMap((pattern)=>{
        ...
        const hasConflictingKeys = pattern.stable_key !== undefined && pattern.key !== undefined && pattern.stable_key !== pattern.key;
        return pattern.status === '已命中' && !hasConflictingKeys && isVerifiedZiweiPatternKey(stableKey) ? [
            stableKey
        ] : [];
    }));
...
export function buildPatternAnalysis(params) {
    ...
    const evaluationContext = palaceDataComplete ? createPatternContext({
        palaces,
        birthYearHeavenlyStem: params.birthYearHeavenlyStem
    }) : null;
    const evaluatedRuleCount = skipped || blockedBySourceAudit || !evaluationContext ? 0 : VERIFIED_PATTERN_RULES.filter((rule)=>canEvaluatePatternRule(rule, evaluationContext)).length;
    const unevaluatedRuleCount = registeredRuleCount - evaluatedRuleCount;
    const acceptedPatterns = skipped || blockedBySourceAudit ? [] : selectVerifiedZiweiPatterns({
        patterns,
        palaces,
        birthTimeLabel: params.birthTimeLabel,
        birthTimeRange: params.birthTimeRange,
    ...
    const patternFactKeys = acceptedPatterns.flatMap((item)=>{
        ...
        return isVerifiedZiweiPatternKey(stableKey) ? [
            stableKey
        ] : [];
    });
    const summaryStatus = skipped || blockedBySourceAudit ? '未生成' : !palaceDataComplete || unevaluatedRuleCount > 0 ? '资料不足' : matchedPatternCount === 0 ? '未命中' : '已完成';
    const analysisStatus = skipped || blockedBySourceAudit ? '未生成' : !palaceDataComplete || unevaluatedRuleCount > 0 ? '资料不足' : matchedPatternCount === 0 ? '未命中' : '已计算';
    const calculationSteps = [
        {
            ...
            result: {
                palaceDataComplete,
                uniquePalaceIndexCount
            },
            promptText: palaceDataComplete ? '已校验完整十二宫及唯一宫位索引' : `当前仅有${palaces.length}项宫位资料或宫位索引不唯一，不执行格局规则评估`,
            sources: [
                '紫微十二宫结构化盘面资料'
            ],
        ...
        },
        {
            ...
            stage: '格局规则评估',
            status: skipped || blockedBySourceAudit ? '未生成' : palaceDataComplete && unevaluatedRuleCount === 0 ? '已计算' : '资料不足',
            dependsOnStepKeys: [
                'ziwei:pattern:calculation:input'
            ],
            inputs: {
                registeredRuleCount,
                palaceDataComplete,
            ...
            promptText: skipped ? '本次明确跳过格局规则评估，未生成格局命中或未命中事实' : blockedBySourceAudit ? `${ZIWEI_PATTERN_AUDIT_NOTICE}，本次调用未接入已校勘登记表` : `已按固定古籍版本逐条评估${evaluatedRuleCount}条可复算规则；${unevaluatedRuleCount}条因必要输入不足未评估；另有${ZIWEI_TRADITIONAL_PATTERN_BOUNDARIES.length}项原典边界未伪造命中`,
            sources: [
        ...
        },
        {
            ...
            promptText: skipped ? '本次明确跳过格局命中事实登记' : blockedBySourceAudit ? '本次调用未使用已校勘登记表，不生成格局命中事实' : `登记${matchedPatternCount}项已校勘格局事实`,
            sources: [
                '逐条规则评估结果与实际命中宫位、星曜'
            ],
            limitation: PATTERN_CALCULATION_LIMITATION
        },
        {
            ...
            status: skipped || blockedBySourceAudit ? '未生成' : palaceDataComplete && unevaluatedRuleCount === 0 ? '已计算' : '资料不足',
            dependsOnStepKeys: [
                PATTERN_MATCHED_FACTS_STEP_KEY
            ],
            inputs: {
                registeredRuleCount,
                evaluatedRuleCount,
    ...
    const counterEvidenceFacts = [
        {
            ...
            promptText: skipped ? '本次明确跳过格局分析，未使用十二宫资料形成格局结论' : palaceDataComplete ? '十二宫资料与宫位索引完整' : '十二宫资料不完整，格局分析必须降级且不得补造命中结果',
            sources: [
                '十二宫输入校验结果'
            ],
        ...
        },
        {
            ...
            type: '登记规则覆盖',
            status: skipped || blockedBySourceAudit ? '未生成' : palaceDataComplete && unevaluatedRuleCount === 0 ? '有可用证据' : '资料不足',
            ownerFactKeys: [
                PATTERN_RULE_STEP_KEY,
                ...patternFactKeys
            ],
            promptText: skipped ? '本次明确跳过登记规则评估，不形成格局覆盖结论' : blockedBySourceAudit ? `${ZIWEI_PATTERN_AUDIT_NOTICE}；不得把空结果解释为没有传统格局` : `当前登记${registeredRuleCount}条已校勘规则，其中已评估${evaluatedRuleCount}条、必要输入不足${unevaluatedRuleCount}条；另登记${ZIWEI_TRADITIONAL_PATTERN_BOUNDARIES.length}项不可唯一复算边界，目录外格局不作判断`,
            sources: [
                '登记规则固定古籍版本、卷次定位与计算条件'
            ],
            limitation: PATTERN_COUNTER_LIMITATION
        },
        {
            ...
            type: '未命中规则边界',
            status: skipped || blockedBySourceAudit ? '未生成' : palaceDataComplete && unevaluatedRuleCount === 0 ? '未命中' : '资料不足',
            ownerFactKeys: [
                PATTERN_MATCHED_FACTS_STEP_KEY,
                ...patternFactKeys
            ],
            promptText: skipped ? '本次明确跳过规则评估，不形成格局未命中结论' : `已评估规则中有${unmatchedRuleCount}条未命中；这不代表命盘没有其他传统格局`,
            sources: [
                '当前登记规则逐条评估结果'
            ],
            limitation: PATTERN_COUNTER_LIMITATION
    ...
    const summaryFact = {
        ...
        inauspiciousPatternCount: acceptedPatterns.filter((item)=>item.kind === 'inauspicious').length,
    ...
    const counterEvidence = counterEvidenceFacts.filter((item)=>item.status !== '有可用证据').map((item)=>item.promptText);
    ...
    return {
        ...
        methodology: {
            notes: [
                `仅执行${VERIFIED_ZIWEI_PATTERN_RULE_COUNT}条具备固定版本、卷次定位与可复算条件的登记规则。`,
                `另登记${ZIWEI_TRADITIONAL_PATTERN_BOUNDARIES.length}项固定版本原典边界，逐项保留名称、来源定位与不能复算的原因。`,
