...
5. 来源对宫有主星；
6. 当前流派允许把对宫主星名称作为辅助语境。

只引用主星名称，而且结果始终保留 `source_palace_id` 和 `context_only` 角色。不得写成“借入后坐守目标宫”，不得搬移亮度、四化、辅星、煞星、宫干或其他属性，不改变目标宫或来源宫的原始事实，也不允许对宫为空后再向别宫追借。

以下任何变化都会撤销对宫语境：目标宫不再为空、对宫关系改变、来源主星改变，或切换到不采用此语境的 profile。撤销是重新运行同一个纯函数的结果，不需要修补历史对象。
...
- 该层保留通用状态词汇，但开发者候选入口只接受三条不可变、可重放的结构候选；它不接受调用方自造格局名。
- 普通已知时辰结果另附固定 Mingyu MIT 版本适配的 55 条机械格局条件与 32 条拒绝边界。它们只作补充证据，普通结果使用中性标签，传统名后置；不改写 `R-ZW-007/008/009` 五主题主结论，不投票、评分或生成事件。
- 未知时辰、家庭广义人际合并主题、缺完整三方四正或缺阶段必要事实都有固定拒绝/回退路线；不得由模型选择候选盘、替宫或补事实。
- “准确”在这里首先指排盘事实绑定正确、规则条件不漏、反例能推翻结论、改判路径透明；它不等于已证明现实预测准确。
- 自动测试覆盖任意候选、错 evidence kind/path、假 ref/hash、同一 fact 跨层复用、缺结构、跨主题、流派隔离、55+32 格局账本、未知时辰关闭和空宫对宫语境撤销等正反例。
