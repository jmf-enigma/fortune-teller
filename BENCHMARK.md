...
当前发布候选必须以本次 [发布审计](docs/RELEASE_AUDIT.md) 中重新运行的测试数量和干净包结果为准，而不是从注册表状态或旧版本数字反推。历史版本的结果不自动沿用到 `0.2.0`。
...
- 太阳时仅在经度、时区和 live registry 明确提供安全 profile 时启用；`0.2.0` 的八字与紫微太阳时 override 均已关闭；
...
`deep` 与 `audit` 还必须包含非空不确定性摘要、逐 claim 推理摘要、替代解释和结构化后续操作。每条传统或解释性 claim 至少引用两个不同且实质相关的事实根；同一事实对象的两个叶字段不能冒充两条事实，加入一个与已引规则无关的事实来凑数量也必须失败。机器校验通过仍不等于自由文本在语义上正确，最终叙事复核不能省略。

紫微阶段 claim 还必须同时引用本命、大限和流年三组事实；只给一个运限宫位或一颗四化不得冒充深入阶段判断。回顾核验必须先冻结判断，再同时保留吻合、不吻合和无法判断，不能按反馈改时辰或改写原结论。
