...
当前发布候选必须以本次 [发布审计](docs/RELEASE_AUDIT.md) 中重新运行的测试数量和干净包结果为准，而不是从注册表状态或旧版本数字反推。历史版本的结果不自动沿用到 `0.5.0`。
...
- 太阳时仅在经度、时区和 live registry 明确提供安全 profile 时启用；`0.5.0` 的八字与紫微太阳时 override 均已关闭；
