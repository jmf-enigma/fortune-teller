...
> 当前版本：`0.5.0`。公开仓库：[jmf-enigma/fortune-teller](https://github.com/jmf-enigma/fortune-teller)。
...
| 方法 | 状态 | 已实现边界与专业深度 |
|---|---|---|
| 四柱八字 | stable，受历法参考限制 | 除四柱、十神与关系外，已计算月令候选、主/中/余气根与显露力量；专业裁决把强弱竞争、格局成败救应、扶抑、调候、通关、病药分轨。调候使用十日干 × 十二月支的 120 条原文提及干筛查入口；数组不代表优先次序，五条月内节气分段及尚未闭合的角色条件保持未决。指定日期按原局冻结 → 大运加入 → 流年加入逐层重跑已登记成格/受损/救应路线。已知时刻可显式选择传统顺逆参数，计算精确起运、24 步大运与立春换年；边界日保持未决。**只有每个实际出生瞬间 UTC 偏移为 `+08:00` 才放行**；其他偏移失败关闭，不手工换算 |
| 紫微斗数 | qualified | `iztro` 默认与中州口径、十二宫、三方四正、四化与五个封闭主题。结果层按整体、事业学业、财富资源、长期关系、身心节奏精确调用本命主题或本命 → 大限 → 流年阶段路线；完整保留主宫、三合、对宫、主星同宫组合与庙旺、六吉六煞、禄存天马、主题四化。未知时辰不从候选盘中选一张；未登记的家庭人际主题也不偷换成另一宫位。阶段路线仍不是流月/日/时、具体事件或完整紫微论断 |
...
`0.5.0` 把同一套“先算对、再闭合推理、最后说人话”的结果层扩到六个已实现体系，而不是继续增加方法数量：

- [15 个来源记录](src/data/source-registry.mjs)覆盖固定计算实现、有限历史术语、八字成败救应/岁运/调候章节，以及紫微宫位、四化、阶段解读边界；
- [36 条机器可读规则](src/data/rule-registry.mjs)声明适用体系、结论范围、最低事实引用、来源束和允许的认识状态；
