...
0.5.0 对这条目标的具体落实是：八字加入 120 条日干×月支调候**原文提及干筛查入口**、根位/月令/刑害细分、可重放通关候选，并把所有已登记成败救应路线在原局、大运、流年三个输入层完整重跑；调候数组不代表优先次序，节气分段与未闭合角色保持未决。紫微继续走封闭五主题和完整三方四正，而不是导入未经逐条复算的 45 格局；西占、塔罗、周易和梅花分别安装自己的有界裁决链和明确拒绝条件。由此可以说本项目在**当前已安装路线的闭合度**上更强，但仍不能说八字知识总量或综合术数深度已经超过 Mingyu 等大型项目。
...
以下各项是 `0.5.0` 需要按 [BENCHMARK.md](../BENCHMARK.md) 与 [RELEASE_AUDIT.md](RELEASE_AUDIT.md) 逐项复核的工程差异。它们只支持具体维度的比较，不支持“整体最好”或“更准”的结论：
...
- 大规模知识库、完整古籍出处索引和分流派解释资料；当前只有 15 个窄来源记录、36 条适用性规则，以及刻意收窄的六体系裁决包；
- 与第一梯队相当的累计测试数量、维护历史和外部使用反馈。
- 独立命理专家对固定 commit/rule-pack 的盲审，以及预注册的现实预测效度研究。

这些差距应公开记录。对 Mingyu 的结论尤其要具体：本项目在当前六条已实现路径的封闭条件链、普通人可读结果、未知资料失败关闭和默认本地隐私上更严格；Mingyu 在八字知识总量、紫微格局/流派数量、术数广度、产品入口和长期维护体量上仍更强。没有依据把局部工程优势写成“总体超过 Mingyu”或“预测更准”。
