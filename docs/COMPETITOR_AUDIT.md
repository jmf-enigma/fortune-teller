...
- [chinese-fortune](https://github.com/ShousenZHANG/chinese-fortune/blob/main/SKILL.md) 会先收集用户关心的事业、感情、财富等议题，并要求把强结论、阶段和盘面依据放进结果。可取之处是“先明确议题、弱结论降级、术语和现代解释分开”。其按既往事件命中反馈再调整用神/格局的闭环容易产生后见偏差和选择性验证，本项目不采用这种自适应校准；若做回顾核对，必须先冻结假设并同时记录命中与未命中。
...
2. 先做两个旗舰结果：紫微的人生主轴，以及用户显式指定日期时的大限/流年阶段；塔罗聚焦当前问题；不靠方法数量掩盖浅解读。
3. 用户结果固定为：直接答案→支持与制约→分领域或当前局面→条件性走向→现实下一步→实际限制。
4. `profile`、warning code、候选/扫描数、哈希、版本、fact/rule/source ID 和 JSON 全部退到“依据与核对（高级）”或技术记录。
5. 所有目标日期显式输入；所有回顾核对先冻结结论，不按用户反馈改盘或只统计命中。
6. 多牌必须有关系综合；紫微目标日期阶段主题必须同时用到本命主题宫、大限和流年，禁止单星或单一周期标签直断事件。
...
- **盲核验而不是追着反馈改口**：像研究流程先写判断和淘汰标准一样，回顾过去事件必须先冻结假设，再同时登记吻合、不吻合和说不清，不利用反馈挑时辰或改规则。
- **后台严格、前台简洁**：像专业评审 Skill 把内部 ledger 与交付意见分开一样，本项目保留规则、来源、warning、哈希和候选账本，但普通用户先看到综合答案和现实下一步。

这些模式已经转化为交互回归测试：目标优先路由、紫微目标日期的有/无时辰分支、技术字段渐进披露、塔罗换问题前明确重抽，以及关键资料变化后旧解读失效。

## 7. 本项目当前可辩护的差异化

以下各项是 `0.2.0` 需要按 [BENCHMARK.md](../BENCHMARK.md) 与 [RELEASE_AUDIT.md](RELEASE_AUDIT.md) 逐项复核的工程差异。它们只支持具体维度的比较，不支持“整体最好”或“更准”的结论：
...
11. **中西一致的工程纪律**：同一审计规则适用于八字、紫微、西占、塔罗和易卦，而不是为某体系降低证据标准。
...
- 大规模知识库、完整古籍出处索引和分流派解释资料；当前只有 10 个窄来源记录、26 条适用性规则，以及一份刻意收窄的紫微解读地图；
